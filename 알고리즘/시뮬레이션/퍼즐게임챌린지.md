...
### 반환값
- 제한 시간 내에 퍼즐을 모두 해결하기 위한 숙련도의 최솟값을 정수로 return 합니다.
...
## 2. 풀이 전략
1. left, right 두 포인트를 가지고 중간값을 계산해서 최소값 구하기
2. 시간 계산하는 함수 필요
3. while, for 문 사용

### 3. 직접 코드 구현(java)
```java
    public int solution(int[] diffs, int[] times, int limit) {
        int answer = 0;
        ...
        while(left<=right) {
            ...
            for(int i = 0; i<diffs.lenght ; i++){
                currentTime += getTime(mid, times[i], times[i-1], diffs[i]);
            }
    ...
    private long getTime(int myLevel, int time_cur, int time_prev, int diff){
        if(diff<=myLevel) return time_cur;
        return (long)(diff-myLevel)*(time_cur + time_prev)+time_cur;
